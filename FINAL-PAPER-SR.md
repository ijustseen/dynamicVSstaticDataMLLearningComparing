...
Вештачка интелигенција у контексту овог рада посматра се као скуп метода који омогућавају аутоматско издвајање обележја из визуелних података и класификацију стања човека.

Савремени квалитет у задацима слика и видеа обезбедиле су методе дубоког учења [5]. За слике су стандард постале CNN архитектуре, где се обележја издвајају хијерархијски: од једноставних ивица ка сложенијим обрасцима [2], [5]. За секвенцијалне податке користе се рекурентни механизми, посебно LSTM, који могу да уважавају зависности између суседних временских корака [3].
...
Рачунарски вид решава задатак интерпретације визуелних информација: од детекције лица до класификације емоције. У практичном FER pipeline-у обично се издвајају фазе:
...
Прелаз са ручно дефинисаних обележја на end-to-end обучиве моделе био је кључни фактор раста квалитета [5], [6].
...
Класични оријентир за категоријално препознавање емоција јесте приступ Пола Екмана и FACS [1]. У примењеним системима користи се ограничен скуп класа, допуњен класом neutral. У текућем експерименту, према датасету, издвојено је пет класа: anger, happy, love, neutral, sad.

Важно је истаћи да препознавање емоција по лицу није „директно мерење унутрашњег стања“. То је статистичка процена на основу визуелних обележја, па коректна интерпретација резултата мора да уважава ограничења података и контекста [6].
...
Резултати FER прегледа показују да temporal modeling често додаје на квалитету, нарочито код сложених емоционалних прелаза [6]. Али у системима са строгим захтевом за FPS статички приступ остаје конкурентан.
...
Извор: Urdu-Multimodal-Emotion-Dataset [4] (коришћен визуелни модалитет).
...
Главне претње валидности и ограничења у овом истраживању проистичу из природе података, начина формирања ознака и изабраних инжењерских решења у pipeline-у. Прво, датасет је прикупљен у одређеном језичко-културном контексту, па се добијени резултати не могу аутоматски пренети на друге популације, услове снимања или типове корисника без засебне провере (ограничење екстерне валидности). Друго, присутан је дисбаланс класа (нпр. Love је мање заступљена у односу на Anger), што може утицати на стабилност учења и профил грешака, нарочито код ређих образаца.

Додатни извор ограничења је детекција лица преко Haar Cascade и усвојени fallback механизам: када детекција не успе, улаз се формира из целог кадра. Такав приступ повећава робусност и спречава губитак узорака, али може уносити шум у обележја (позадина, делимично лице), што се одражава на квалитет и стабилност предвиђања. Такође, поређење је намерно ограничено на две архитектуре (ResNet-18 и ResNet-18+LSTM): то омогућава јасно и репродуцибилно испитивање ефекта временског контекста у контролисаним условима, али не представља потпуни обухват савремених temporal модела.

Коначно, важно је уважити и ризик субјективне неодређености ознака емоција. Категоријална анотација је по природи приближна, посебно код прелазних и амбивалентних израза, па део грешака модела може одражавати неодређеност у самој разметци, а не искључиво слабост архитектуре. Због тога резултате треба тумачити као мерење способности модела да репродукује ознаке датасета у задатом протоколу, уз наведена ограничења.
...
[2] He, K., Zhang, X., Ren, S., Sun, J., (2016), Deep Residual Learning for Image Recognition, CVPR, 770-778, DOI: 10.1109/CVPR.2016.90.

[3] Hochreiter, S., Schmidhuber, J., (1997), Long Short-Term Memory, Neural Computation, 9(8), 1735-1780, DOI: 10.1162/neco.1997.9.8.1735.

[4] Hugging Face, Maisum-Abbas-123/Urdu-Multimodal-Emotion-Dataset, https://huggingface.co/datasets/Maisum-Abbas-123/Urdu-Multimodal-Emotion-Dataset (април 2026).

[5] LeCun, Y., Bengio, Y., Hinton, G., (2015), Deep learning, Nature, 521(7553), 436-444, DOI: 10.1038/nature14539.

[6] Li, S., Deng, W., (2022), Deep Facial Expression Recognition: A Survey, IEEE Transactions on Affective Computing, 13(3), 1195-1215, DOI: 10.1109/TAFFC.2020.2981446.
